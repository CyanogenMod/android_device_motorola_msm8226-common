...
static char *camera_fixup_getparams(int id, const char *settings)
{
    ...
    if (id == FRONT_CAMERA) {
        if (device == FALCON || device == PEREGRINE) {
            ...
            /*
             * The HW detection causes a stream of errors and the camera has a
             * fixed focus, so just disable it.
             */
            params.set(android::CameraParameters::KEY_MAX_NUM_DETECTED_FACES_HW, 5);
        }
    }
